...
- Name: Ryan Lian (display name everywhere; "chunren lian" survives only as an SEO keyword in `Meta.tsx`)
- Title line: Full-Stack Engineer · Commerce Builder · AI Product Developer
- Education: B.A. Computer Science, Alma College
...
- Repo: https://github.com/LianCr/agentdesk · Live: https://agentdesk-acme307.vercel.app (the `agentdesk-one` Vercel URL also resolves but is not the canonical one)
- One-liner: Bilingual insurance copilot — ask in Chinese, get an answer grounded in the English source documents with page number and verbatim quote attached; citations are injected and verified by code, not generated by the model.
- Case study source: the repo README, whose "claim → how it's enforced" structure the MDX mirrors. Numbers to keep accurate: 369 unit / 77 database / 104 Playwright tests; frozen evals 28/30 cases, 17/17 and 19/19 hard gates; knowledge base frozen at 3 docs / 20 pages / 45 chunks.
- Tags: Next.js · TypeScript · Supabase · pgvector · OpenAI · Vercel AI SDK · zod · Playwright
...
Source of truth is `src/config/Experience.tsx` (array order = display order; index 0
is the homepage "Featured Experience"). Four entries, from the August 2026 resume —
the earlier Ryzlink/ZentraPay history was replaced wholesale:

1. **Wayfair** — Full Stack Developer — Irvine, CA — Jun 2024 – Present (current).
   Cart and checkout domain: Node.js/TypeScript BFF on Apollo GraphQL,
   server-authoritative cart services, React 18 + Redux Toolkit front end,
   Docker/AWS (RDS, CloudWatch). Resume's 6 bullets used as-is.
2. **SoFi** — Full Stack Developer — San Francisco, CA (Remote) — Aug 2023 – Jun 2024.
   GraphQL aggregation layer, Redis caching (−40% latency), WebSockets
   transaction updates, D3 visualizations. Resume's 4 bullets expanded to 6
   (added Material UI/Redux Toolkit views, Jest coverage — both from the
   resume's own tech line).
3. **Meijer** — Software Developer — Grand Rapids, MI — Apr 2022 – Jun 2023.
   Angular workforce scheduling: Reactive Forms, RBAC/JWT, Node.js REST sync.
   3 bullets expanded to 6 (PrimeNG/SASS components, RxJS state, Jest tests).
4. **Alibaba Group** — UI Developer — Beijing, China — Feb 2020 – Mar 2022.
   React/TypeScript vendor administration portal, Webpack code splitting.
   2 bullets expanded to 6 (Redux state, REST integration, responsive
   Bootstrap 5/SASS layouts, cross-team collaboration).

Expansion rule that produced the extra bullets: decompose only what the resume's
own bullets and Technologies lines already claim — no new metrics, no invented
tech. Bullets use the card's `*text*` → bold convention.
...
A tech with no icon is fine and degrades on purpose: `ProjectCard` and `ExperienceCard` both render an outline `Badge` with the name (the `ExperienceCard` fallback was added when the new experience entries brought in unregistered tags — before that it rendered an empty hoverable box). Currently text-only: Playwright, Polymarket APIs, Tavily, pgvector, OpenAI, Vercel AI SDK, zod, JWT, RxJS, WebSockets, PrimeNG.

## Content status

Identity, experience, all three case studies, and all imagery are done. Assets
in place: `public/assets/avatar.png`,
`public/company/{wayfair,sofi,meijer,alibaba}.png`,
`public/project/{smart-money-decoder,agentdesk,deallens}.png`,
`public/meta/opengraph-image.png`.

Asset provenance worth remembering: the Wayfair/SoFi/Meijer logos are Google
s2 favicons (Clearbit's logo API is dead — DNS no longer resolves); the Alibaba
mark is the simple-icons `alibabadotcom` glyph filled `#FF6A00` and rendered to
PNG, because every favicon source only served 16–48px for them. The AgentDesk
cover is a screenshot of the live demo (the repo README has no images). The OG
image is generated from an HTML mock styled after the original card ("Hey, I'm
Ryan — A Full-Stack Engineer.") and screenshotted with headless Chrome at
1200×630 @1.4x — the old file had "Chunren" baked into the pixels.

Project display order is explicit via `order:` in MDX frontmatter (Smart Money
Decoder 1, AgentDesk 2, DealLens 3), because chronology alone would reorder
them. Homepage Featured shows the first two.
